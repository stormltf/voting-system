...
**Railway** 提供 $5/月免费额度，可同时部署前端、后端和数据库。

#### 步骤：

1. 注册 [Railway](https://railway.app)
2. 创建新项目，选择 "Deploy from GitHub repo"
3. 添加 MySQL 服务（Add New Service → Database → MySQL）
4. 部署后端：
   - Add New Service → GitHub Repo → 选择 `backend` 目录
   - 设置环境变量（从 MySQL 服务获取连接信息）
5. 部署前端：
   - Add New Service → GitHub Repo → 选择 `frontend` 目录
   - 设置 `NEXT_PUBLIC_API_URL` 为后端地址

---

### 分离部署方案：Vercel + Render + PlanetScale
...
# 2. 创建数据库 "voting_system"
# 3. 获取连接字符串
# 4. 在 PlanetScale 控制台执行 database/schema.sql
```
...
| `NODE_ENV` | production |
| `PORT` | 3001 |
| `DB_HOST` | (PlanetScale 主机) |
| `DB_USER` | (PlanetScale 用户) |
| `DB_PASSWORD` | (PlanetScale 密码) |
| `DB_NAME` | voting_system |
| `DB_SSL` | true |
| `JWT_SECRET` | (随机字符串) |

> 注意：Render 免费版 15 分钟无请求会休眠，首次访问需等待约 30 秒
